...
//    }
  ...
  def editCommand(what: String, editor: Option[String]): Result = {
    ...
    if (isLast || !isNum) {
      ...
      val text = history match {
        case jlh: JLineHistory => jlh.entries(index).asScala.take(len) map (f => f.value) mkString "\n"
  ...
  private def loopPostInit() {
    in.asInstanceOf[JLineReader].consoleReader.postInit
    ...
    in match {
      case x: JLineReader => {
        System.out.println("JLineReader!!!")
        x.consoleReader.postInit
      }
      case _              =>System.out.println("NOT NO NOT JLineReader!!!")
    }
  }
